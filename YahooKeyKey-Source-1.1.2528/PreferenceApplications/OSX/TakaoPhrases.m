    ...
    NSAlert *alert =
        [NSAlert alertWithMessageText:LFLSTR(@"Unable to export database.")
            ...
            informativeTextWithFormat:
                LFLSTR(@"If you are not runnung Yahoo! KeyKey, you are not "
...
//\"com.apple.preference.internet\"\nend tell"; 		NSAppleScript *script =
//[[NSAppleScript alloc] initWithSource:source]; 		[script autorelease]; 		[script
//executeAndReturnError:nil];
//	}
//	else if (returnCode == NSAlertAlternateReturn) {
//		// The API to launch the web browser to open the MobileMe website
//to register a new account. 		[DMMemberAccount signUpNewMemberWithApplicationID:
//OPENVANILLA_DOTMAC_ID];
//	}
//}
...
//		NSAlert *alert = [NSAlert alertWithMessageText:LFLSTR(@"Unable to
//login MobileMe.") defaultButton:LFLSTR(@"Open MobileMe Preference")
...
- (IBAction)exportDatabaseToDotMac:(id)sender {
  ...
  //otherButton:nil informativeTextWithFormat:LFLSTR(@"If you are not runnung
  //Yahoo! KeyKey, you are not able to export your database.")]; 			[alert
  ...
  //	@catch(NSException *e) {
  //		NSAlert *alert = [NSAlert alertWithMessageText:LFLSTR(@"Unable to
  //export database.") defaultButton:LFLSTR(@"OK") alternateButton:nil
  //otherButton:nil informativeTextWithFormat:LFLSTR(@"Unknown errors
  //happened.")]; 		[alert beginSheetModalForWindow:window modalDelegate:self
  //didEndSelector:nil contextInfo:nil]; 		return;
  //	}
  //
  ...
  //happend.")]; 		[alert beginSheetModalForWindow:window modalDelegate:self
  //didEndSelector:nil contextInfo:nil]; 		return;
  //	}
  //
  ...
  //			NSAlert *alert = [NSAlert alertWithMessageText:LFLSTR(@"iDisk
  //service is not available.") defaultButton:LFLSTR(@"OK") alternateButton:nil
  //otherButton:nil informativeTextWithFormat:LFLSTR(@"You are not able to use
  //the iDisk service, please check the status of your MobileMe account and try
  //again.")]; 			[alert beginSheetModalForWindow:window modalDelegate:self
  //didEndSelector:nil contextInfo:nil]; 			return;
  //		}
  //		DMiDiskSession *mySession = [DMiDiskSession
  //iDiskSessionWithAccount:myMemberAccount]; 		[mySession setDelegate:self];
  //		NSData *messageData = [NSData dataWithContentsOfFile:path];
  //		_transactionType = tUpload;
  //		DMTransaction *uploadTransaction = [mySession putData:messageData
  //toPath:@"/Documents/Yahoo! KeyKey Phrases.txt"]; 		if (uploadTransaction !=
  //nil) {
  //			[[NSApplication sharedApplication] beginSheet:_progressWindow
  //modalForWindow:window modalDelegate:self didEndSelector:nil
  //contextInfo:nil];
  //			[_progressIndicator startAnimation:self];
  //
  //			if (_transactionTimer)
  //				_transactionTimer = nil;
  //			_transactionTimer = [NSTimer scheduledTimerWithTimeInterval:0.1
  //target:self selector:@selector(transactionTimer:) userInfo:uploadTransaction
  //repeats:YES];
  //		}
  //	} else {
  //		// handle error
...
- (IBAction)importDatabaseFromDotMac:(id)sender {
  ...
  //		ovService = [NSConnection
  //rootProxyForConnectionWithRegisteredName:OPENVANILLA_DO_CONNECTION_NAME
  //host:nil]; 		if (!ovService) { 			NSAlert *alert = [NSAlert
  //alertWithMessageText:LFLSTR(@"Unable to import database.")
  //defaultButton:LFLSTR(@"OK") alternateButton:nil otherButton:nil
  //informativeTextWithFormat:LFLSTR(@"If you are not runnung Yahoo! KeyKey, you
  //are not able to import your database.")]; 			[alert
  //beginSheetModalForWindow:window modalDelegate:self didEndSelector:nil
  //contextInfo:nil]; 			return;
  //		}
  //	}
  //	@catch(NSException *e) {
  //		NSAlert *alert = [NSAlert alertWithMessageText:LFLSTR(@"Unable to
  //import database.") defaultButton:LFLSTR(@"OK") alternateButton:nil
  ...
  //		DMTransaction *downloadTransaction = [mySession
  //getDataAtPath:@"/Documents/Yahoo! KeyKey Phrases.txt"]; 		if
  ...
  //				_transactionTimer = nil;
  //			_transactionTimer = [NSTimer scheduledTimerWithTimeInterval:0.1
  //target:self selector:@selector(transactionTimer:)
  //userInfo:downloadTransaction repeats:YES];
  //		}
  //	} else {
  //		// handle error
  //	}
...
//rootProxyForConnectionWithRegisteredName:OPENVANILLA_DO_CONNECTION_NAME
//host:nil]; 				if (ovService) { 					[ovService
//setProtocolForProxy:@protocol(OpenVanillaService)]; 					NSString *path =
//[ovService randomTemporaryFilenameWithFullpath]; 					[result writeToFile:path
//atomically:YES]; 					NSAlert *alert = nil; 					if ([ovService
//importUserPhraseDBFromFile:path]) 						alert = [NSAlert
...
//customized phrases database from your iDisk.")]; 					else 						alert = [NSAlert
...
//to import the downloaded file, your backup on iDisk may be corrupt.")]; 					[alert
//beginSheetModalForWindow:window modalDelegate:self didEndSelector:nil
//contextInfo:nil];
//				}
//			}
...
//defaultButton:LFLSTR(@"OK") alternateButton:nil otherButton:nil
//informativeTextWithFormat:@""]; 				[alert beginSheetModalForWindow:window
...
//to iDisk.")]; 				[alert beginSheetModalForWindow:window modalDelegate:self
...
//			errorDescription = LFLSTR(@"Unable to interact with MobileMe
//because of a networking error."); 			break; 		case kDMResourceNotFound:
//			errorDescription = LFLSTR(@"Yahoo! KeyKey user phrases
//database was not found on your iDisk."); 			break; 		case kDMServiceBusy:
//			errorDescription = LFLSTR(@"The iDisk servie is now busy,
//please wait for a while and try again."); 			break; 		case kDMInvalidParameter:
//			errorDescription = LFLSTR(@"A user-provided parameter was
//invalid."); 			break; 		default: 			errorDescription = LFLSTR(@"Unknown error
//happend."); 			break;
//	}
  ...
  if (![[NSWorkspace sharedWorkspace] openFile:phraseEditorPath]) {
    [[NSWorkspace sharedWorkspace]
         launchAppWithBundleIdentifier:
             @"com.yahoo.inputmethod.KeyKey.PhraseEditor"
