...
+ (NSBezierPath *)bezelPathWithRect:(NSRect)boundingRect radius:(float)radius {
  ...
  [path appendBezierPathWithArcFromPoint:NSMakePoint(x2, y1)
                                 toPoint:NSMakePoint(x2, y1 + radius)
                                  radius:radius];
  [path lineToPoint:NSMakePoint(x2, y2 - radius)];
  [path appendBezierPathWithArcFromPoint:NSMakePoint(x2, y2)
                                 toPoint:NSMakePoint(x2 - radius, y2)
                                  radius:radius];
  [path lineToPoint:NSMakePoint(x1 + radius, y2)];
  [path appendBezierPathWithArcFromPoint:NSMakePoint(x1, y2)
                                 toPoint:NSMakePoint(x1, y2 - radius)
                                  radius:radius];
  [path lineToPoint:NSMakePoint(x1, y1 + radius)];
  [path appendBezierPathWithArcFromPoint:NSMakePoint(x1, y1)
                                 toPoint:NSMakePoint(x1 + radius, y1)
                                  radius:radius];
  [path closePath];
