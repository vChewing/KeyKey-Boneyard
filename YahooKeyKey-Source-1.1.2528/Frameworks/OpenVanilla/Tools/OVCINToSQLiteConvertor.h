...
class OVCINToSQLiteConvertor {
 protected:
  static bool InsertKeyValue(OVFastKeyValuePairMap* map,
 ...
 public:
  static bool Convert(OVCINDataTable* table, OVSQLiteConnection* connection,
    ...
    if (connection->execute("CREATE INDEX %Q on %Q (key)", indexName.c_str(),
                            nameStr) != SQLITE_OK)
      return false;
