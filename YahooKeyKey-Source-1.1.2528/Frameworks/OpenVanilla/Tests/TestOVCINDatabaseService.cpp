...
void end() {
  g_benchmark.stop();
  cout << g_benchmarkItem << "ended, elapsed time "
       << g_benchmark.elapsedSeconds() << endl;
}
...
int main(int argc, char* argv[]) {
  ...
  for (iter = tables.begin(); iter != tables.end(); ++iter) {
    ...
    OVKeyValueDataTableInterface* table =
        dataService.createKeyValueDataTableInterface(*iter);
    ...
    if (result.size()) {
      ...
      for (vector<string>::iterator iter = result.begin(); iter != result.end();
           ++iter)
        cout << *iter << ", ";
      cout << endl;
    } else {
