...
string BuildFailureMessage(string const& file, int const line,
...
void XmlTestReporter::BeginResults(std::ostream& os, int const totalTestCount,
                                   int const failedTestCount,
                                   int const failureCount,
                                   float const secondsElapsed) {
  os << "<unittest-results"
...
void XmlTestReporter::AddFailure(std::ostream& os,
  ...
  for (DeferredTestResult::FailureVec::const_iterator it =
    ...
    string const message =
        BuildFailureMessage(result.failureFile, it->first, escapedMessage);
