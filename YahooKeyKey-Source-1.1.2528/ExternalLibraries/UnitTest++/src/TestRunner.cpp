...
struct True {
  bool operator()(const Test* const /*test*/) const { return true; }
};
...
int RunAllTests(TestReporter& reporter, TestList const& list,
...
TestReporter* TestRunner::TestContext::GetReporter() const { return reporter; }

TestResults* TestRunner::TestContext::GetResult() const { return result; }

Timer* TestRunner::TestContext::GetTimer() const { return timer; }
...
int TestRunner::Finish(TestContext& context) const {
  ...
  context.GetReporter()->ReportSummary(
      result->GetTotalTestCount(), result->GetFailedTestCount(),
      result->GetFailureCount(), secondsElapsed);

  return result->GetFailureCount();
}

bool TestRunner::IsTestInSuite(Test* const curTest,
                               char const* suiteName) const {
  return (suiteName == 0) ||
         !std::strcmp(curTest->m_details.suiteName, suiteName);
}
