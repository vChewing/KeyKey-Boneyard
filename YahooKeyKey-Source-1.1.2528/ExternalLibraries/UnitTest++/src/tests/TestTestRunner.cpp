...
TEST_FIXTURE(TestRunnerFixture, TestFinishIsCalledWithCorrectTime) {
  ...
  RunAllTests(reporter, list, 0);
  CHECK(reporter.lastFinishedTestTime >= 0.005f &&
...
TEST_FIXTURE(TestRunnerFixture, SlowTestHasCorrectFailureInformation) {
  ...
  CHECK(
      std::strstr(reporter.lastFailedMessage, "Global time constraint failed"));
...
TEST_FIXTURE(TestRunnerFixture,
             TestRunnerOnlyRunsTestsInSpecifiedSuiteAndThatPassPredicate) {
