...
TestList list1;
TEST_EX(DummyTest, list1) { (void)testResults_; }
...
TestList list2;
TEST_FIXTURE_EX(ThrowingThingie, DummyTestName, list2) { (void)testResults_; }
...
SUITE(TestSuite1) {
  TEST(SimilarlyNamedTestsInDifferentSuitesWork) { (void)testResults_; }
...
TEST_EX(MacroTestHelper1, macroTestList1) { (void)testResults_; }
...
TEST_FIXTURE_EX(FixtureCtorThrows, FixtureCtorThrowsTestName,
                throwingFixtureTestList1) {
  (void)testResults_;
}
...
TEST_FIXTURE_EX(FixtureDtorThrows, FixtureDtorThrowsTestName,
                throwingFixtureTestList2) {
  (void)testResults_;
}
...
// to compile and link successfuly (TestTestSuite.cpp has suite with the same
// name) Note: we are outside of the anonymous namespace
SUITE(SameTestSuite) {
  TEST(DummyTest1) { (void)testResults_; }
}
