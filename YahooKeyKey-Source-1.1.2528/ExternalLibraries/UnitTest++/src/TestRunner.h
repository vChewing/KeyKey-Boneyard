...
int RunAllTests(TestReporter& reporter, TestList const& list,
                char const* suiteName, int maxTestTimeInMs = 0);
...
class TestRunner {
 public:
  ...
  template <class Predicate>
  int RunAllTestsIf(TestReporter& reporter, TestList const& list,
                    char const* suiteName, const Predicate& predicate,
                    int maxTestTimeInMs = 0) const {
    TestContext context(&reporter);
 ...
 private:
  ...
  template <class Predicate>
  int RunAllTestsIf(TestContext& context, TestList const& list,
                    char const* suiteName, const Predicate& predicate,
                    int maxTestTimeInMs = 0) const {
...
// Predicate version of RunAllTests(TestReporter& reporter, TestList const&
// list, char const* suiteName, int maxTestTimeInMs = 0)
template <class Predicate>
int RunAllTestsIf(TestReporter& reporter, TestList const& list,
                  char const* suiteName, const Predicate& predicate,
                  int maxTestTimeInMs = 0) {
  TestRunner runner;
  return runner.RunAllTestsIf(reporter, list, suiteName, predicate,
                              maxTestTimeInMs);
}
