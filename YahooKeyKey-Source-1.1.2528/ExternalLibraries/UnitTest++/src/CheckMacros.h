...
#define CHECK_EQUAL(expected, actual)                                        \
  do {                                                                       \
    try {                                                                    \
      UnitTest::CheckEqual(testResults_, expected, actual,                   \
...
#define CHECK_ARRAY_EQUAL(expected, actual, count)                           \
  do {                                                                       \
    try {                                                                    \
      UnitTest::CheckArrayEqual(testResults_, expected, actual, count,       \
                                UnitTest::TestDetails(m_details, __LINE__)); \
    } catch (...) {                                                          \
      testResults_.OnTestFailure(                                            \
          UnitTest::TestDetails(m_details, __LINE__),                        \
...
#define CHECK_ARRAY2D_CLOSE(expected, actual, rows, columns, tolerance)        \
  do {                                                                         \
    try {                                                                      \
      UnitTest::CheckArray2DClose(testResults_, expected, actual, rows,        \
                                  columns, tolerance,                          \
                                  UnitTest::TestDetails(m_details, __LINE__)); \
    } catch (...) {                                                            \
      testResults_.OnTestFailure(                                              \
