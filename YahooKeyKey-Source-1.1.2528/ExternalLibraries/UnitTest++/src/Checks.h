...
void CheckArrayEqual(TestResults& results, Expected const& expected,
                     ...
                     TestDetails const& details) {
  bool equal = true;
  for (int i = 0; i < count; ++i) equal &= (expected[i] == actual[i]);

  if (!equal) {
    UnitTest::MemoryOutStream stream;
    stream << "Expected [ ";
    for (int i = 0; i < count; ++i) stream << expected[i] << " ";
    stream << "] but was [ ";
    for (int i = 0; i < count; ++i) stream << actual[i] << " ";
    stream << "]";
...
void CheckArray2DClose(TestResults& results, Expected const& expected,
  ...
  if (!equal) {
    ...
    for (int i = 0; i < rows; ++i) {
      stream << "[ ";
      for (int j = 0; j < columns; ++j) stream << expected[i][j] << " ";
      stream << "] ";
    }
    stream << "] +/- " << tolerance << " but was [ ";
    for (int i = 0; i < rows; ++i) {
      stream << "[ ";
      for (int j = 0; j < columns; ++j) stream << actual[i][j] << " ";
      stream << "] ";
    }
    stream << "]";
