...
static void basic_setup(void) {
  ...
  if (parser == NULL) fail("Parser not created.");
}
...
static void _xml_failure(XML_Parser parser, const char *file, int line) {
  ...
  sprintf(buffer,
          "    %d: %s (line %" XML_FMT_INT_MOD "u, offset %" XML_FMT_INT_MOD
...
static void _expect_failure(char *text, enum XML_Error errorCode,
                            char *errorMessage, char *file, int lineno) {
  if (XML_Parse(parser, text, strlen(text), XML_TRUE) == XML_STATUS_OK)
    /* Hackish use of _fail_unless() macro, but let's us report
       the right filename and line number. */
    _fail_unless(0, file, lineno, errorMessage);
  if (XML_GetErrorCode(parser) != errorCode) _xml_failure(parser, file, lineno);
}
...
static void XMLCALL dummy_start_doctype_handler(void *userData,
...
static void XMLCALL dummy_entity_decl_handler(
    void *userData, const XML_Char *entityName, int is_parameter_entity,
    const XML_Char *value, int value_length, const XML_Char *base,
    const XML_Char *systemId, const XML_Char *publicId,
    const XML_Char *notationName) {}

static void XMLCALL dummy_notation_decl_handler(void *userData,
                                                const XML_Char *notationName,
                                                const XML_Char *base,
                                                const XML_Char *systemId,
                                                const XML_Char *publicId) {}

static void XMLCALL dummy_element_decl_handler(void *userData,
                                               const XML_Char *name,
                                               XML_Content *model) {}

static void XMLCALL dummy_attlist_decl_handler(
    void *userData, const XML_Char *elname, const XML_Char *attname,
    const XML_Char *att_type, const XML_Char *dflt, int isrequired) {}

static void XMLCALL dummy_comment_handler(void *userData,
                                          const XML_Char *data) {}

static void XMLCALL dummy_pi_handler(void *userData, const XML_Char *target,
                                     const XML_Char *data) {}

static void XMLCALL dummy_start_element(void *userData, const XML_Char *name,
                                        const XML_Char **atts) {}
...
START_TEST(test_nul_byte) {
  ...
  if (XML_GetErrorCode(parser) != XML_ERROR_INVALID_TOKEN) xml_failure(parser);
}
END_TEST
...
static void XMLCALL accumulate_characters(void *userData, const XML_Char *s,
                                          int len) {
  CharData_AppendXMLChars((CharData *)userData, s, len);
}

static void XMLCALL accumulate_attribute(void *userData, const XML_Char *name,
                                         const XML_Char **atts) {
...
static void _run_character_check(XML_Char *text, XML_Char *expected,
...
static void _run_attribute_check(XML_Char *text, XML_Char *expected,
                                 const char *file, int line) {
  CharData storage;
...
START_TEST(test_danish_latin1) {
  ...
  run_character_check(
      text, "J\xC3\xB8rgen \xC3\xA6\xC3\xB8\xC3\xA5\xC3\x86\xC3\x98\xC3\x85");
}
END_TEST
...
START_TEST(test_french_charref_hexidecimal) {
  char *text =
      "<?xml version='1.0' encoding='iso-8859-1'?>\n"
      "<doc>&#xE9;&#xE8;&#xE0;&#xE7;&#xEA;&#xC8;</doc>";
  run_character_check(text, "\xC3\xA9\xC3\xA8\xC3\xA0\xC3\xA7\xC3\xAA\xC3\x88");
}
END_TEST
...
START_TEST(test_utf16_le_epilog_newline) {
  ...
  if (XML_Parse(parser, text, first_chunk_bytes, XML_FALSE) == XML_STATUS_ERROR)
    xml_failure(parser);
  else {
    enum XML_Status rc;
    rc = XML_Parse(parser, text + first_chunk_bytes,
                   sizeof(text) - first_chunk_bytes - 1, XML_TRUE);
    if (rc == XML_STATUS_ERROR) xml_failure(parser);
  }
}
END_TEST
...
static void XMLCALL start_element_event_handler2(void *userData,
                                                 const XML_Char *name,
                                                 const XML_Char **attr) {
  CharData *storage = (CharData *)userData;
  char buffer[100];
  ...
  sprintf(buffer,
          "<%s> at col:%" XML_FMT_INT_MOD "u line:%" XML_FMT_INT_MOD "u\n",
          name, XML_GetCurrentColumnNumber(parser),
          XML_GetCurrentLineNumber(parser));
  CharData_AppendString(storage, buffer);
}

static void XMLCALL end_element_event_handler2(void *userData,
                                               const XML_Char *name) {
  CharData *storage = (CharData *)userData;
  char buffer[100];
...
static void XMLCALL end_element_event_handler(void *userData,
                                              const XML_Char *name) {
  CharData *storage = (CharData *)userData;
...
static int is_whitespace_normalized(const XML_Char *s, int is_cdata) {
  ...
  while (*s) {
    if (*s == ' ')
      ...
      if (at_start) {
        at_start = 0;
        if (blanks && !is_cdata) /* illegal leading blanks */
          return 0;
...
static void XMLCALL check_attr_contains_normalized_whitespace(
    void *userData, const XML_Char *name, const XML_Char **atts) {
  int i;
  for (i = 0; atts[i] != NULL; i += 2) {
    const XML_Char *attrname = atts[i];
    const XML_Char *value = atts[i + 1];
    if (strcmp("attr", attrname) == 0 || strcmp("ents", attrname) == 0 ||
        strcmp("refs", attrname) == 0) {
      if (!is_whitespace_normalized(value, 0)) {
        char buffer[256];
        sprintf(buffer, "attribute value not normalized: %s='%s'", attrname,
                value);
        fail(buffer);
      }
    }
...
START_TEST(test_xmldecl_misplaced) {
  expect_failure(
      ...
      "<?xml version='1.0'?>\n"
      "<a/>",
      XML_ERROR_MISPLACED_XML_PI, "failed to report misplaced XML declaration");
}
END_TEST

/* Regression test for SF bug #584832. */
static int XMLCALL UnknownEncodingHandler(void *data, const XML_Char *encoding,
                                          XML_Encoding *info) {
...
static int XMLCALL external_entity_loader_set_encoding(
...
START_TEST(test_ext_entity_set_encoding) {
  ...
  XML_SetExternalEntityRefHandler(parser, external_entity_loader_set_encoding);
  run_character_check(text, "\xC3\xA9");
}
END_TEST
...
static int XMLCALL external_entity_loader(XML_Parser parser,
...
START_TEST(test_dtd_default_handling) {
  ...
  XML_SetDoctypeDeclHandler(parser, dummy_start_doctype_handler,
...
static void clearing_aborting_character_handler(void *userData,
...
START_TEST(test_stop_parser_between_char_data_calls) {
  ...
  */
  ...
  if (XML_Parse(parser, text, strlen(text), XML_TRUE) != XML_STATUS_ERROR)
    xml_failure(parser);
  if (XML_GetErrorCode(parser) != XML_ERROR_ABORTED) xml_failure(parser);
}
END_TEST
...
START_TEST(test_suspend_parser_between_char_data_calls) {
  ...
  */
  ...
  if (XML_Parse(parser, text, strlen(text), XML_TRUE) != XML_STATUS_SUSPENDED)
    xml_failure(parser);
  if (XML_GetErrorCode(parser) != XML_ERROR_NONE) xml_failure(parser);
}
END_TEST
...
static void namespace_teardown(void) { basic_teardown(); }
...
*/
static void XMLCALL triplet_start_checker(void *userData, const XML_Char *name,
                                          const XML_Char **atts) {
...
START_TEST(test_return_ns_triplet) {
  ...
  char *elemstr[] = {"http://expat.sf.net/ e foo",
...
static void XMLCALL overwrite_start_checker(void *userData,
                                            const XML_Char *name,
                                            const XML_Char **atts) {
  CharData *storage = (CharData *)userData;
...
static void XMLCALL overwrite_end_checker(void *userData,
                                          const XML_Char *name) {
  CharData *storage = (CharData *)userData;
...
static void run_ns_tagname_overwrite_test(char *text, char *result) {
  CharData storage;
  CharData_Init(&storage);
  XML_SetUserData(parser, &storage);
  XML_SetElementHandler(parser, overwrite_start_checker, overwrite_end_checker);
  if (XML_Parse(parser, text, strlen(text), XML_TRUE) == XML_STATUS_ERROR)
    xml_failure(parser);
...
static void XMLCALL start_element_fail(void *userData, const XML_Char *name,
                                       const XML_Char **atts) {
  /* We should never get here. */
  fail("should never reach start_element_fail()");
}

static void XMLCALL start_ns_clearing_start_element(void *userData,
...
static int XMLCALL external_entity_handler(XML_Parser parser,
                                           const XML_Char *context,
                                           const XML_Char *base,
                                           const XML_Char *systemId,
                                           const XML_Char *publicId) {
...
START_TEST(test_ns_prefix_with_empty_uri_4) {
  ...
  char *elemstr[] = {"http://xml.libexpat.org/ doc prefix"};
  XML_SetReturnNSTriplet(parser, XML_TRUE);
  XML_SetUserData(parser, elemstr);
...
int main(int argc, char *argv[]) {
  ...
  if (verbosity != CK_SILENT) printf("Expat version: %s\n", XML_ExpatVersion());
