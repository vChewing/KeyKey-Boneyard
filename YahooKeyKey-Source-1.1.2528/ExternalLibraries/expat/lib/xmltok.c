...
static void PTRCALL utf8_toUtf8(const ENCODING *enc, const char **fromP,
  ...
  if (fromLim - *fromP > toLim - *toP) {
    ...
    for (fromLim = *fromP + (toLim - *toP); fromLim > *fromP; fromLim--)
      if (((unsigned char)fromLim[-1] & 0xc0) != 0x80) break;
  }
  for (to = *toP, from = *fromP; from != fromLim; from++, to++) *to = *from;
  *fromP = from;
  *toP = to;
}
...
static void PTRCALL latin1_toUtf8(const ENCODING *enc, const char **fromP,
  ...
  for (;;) {
    unsigned char c;
    if (*fromP == fromLim) break;
    c = (unsigned char)**fromP;
    if (c & 0x80) {
      if (toLim - *toP < 2) break;
