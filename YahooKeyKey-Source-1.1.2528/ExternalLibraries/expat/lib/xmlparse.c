...
XML_Bool XMLCALL XML_ParserReset(XML_Parser parser,
  ...
  if (parentParser) return XML_FALSE;
  ...
  if (unknownEncodingRelease) unknownEncodingRelease(unknownEncodingData);
  poolClear(&tempPool);
  poolClear(&temp2Pool);
  parserInit(parser, encodingName);
...
enum XML_Status XMLCALL XML_SetEncoding(XML_Parser parser,
  ...
  */
  ...
  if (encodingName == NULL)
    ...
    if (!protocolEncodingName) return XML_STATUS_ERROR;
  }
...
void XMLCALL XML_ParserFree(XML_Parser parser) {
  ...
  for (;;) {
    ...
    if (entityList == NULL) {
      if (freeInternalEntities == NULL) break;
...
void XMLCALL XML_SetReturnNSTriplet(XML_Parser parser, int do_nst) {
  /* block after XML_Parse()/XML_ParseBuffer() has been called */
  if (ps_parsing == XML_PARSING || ps_parsing == XML_SUSPENDED) return;
...
int XMLCALL XML_SetParamEntityParsing(XML_Parser parser,
  ...
  /* block after XML_Parse()/XML_ParseBuffer() has been called */
  if (ps_parsing == XML_PARSING || ps_parsing == XML_SUSPENDED) return 0;
#ifdef XML_DTD
...
XML_Index XMLCALL XML_GetCurrentByteIndex(XML_Parser parser) {
  if (eventPtr) return parseEndByteIndex - (parseEndPtr - eventPtr);
  return -1;
}

int XMLCALL XML_GetCurrentByteCount(XML_Parser parser) {
  if (eventEndPtr && eventPtr) return (int)(eventEndPtr - eventPtr);
  return 0;
}
    ...
    *eventEndPP = next;
    switch (tok) {
      ...
      case XML_TOK_EMPTY_ELEMENT_WITH_ATTS: {
        ...
        if (tagLevel == 0) return epilogProcessor(parser, next, end, nextPtr);
        break;
      ...
      case XML_TOK_CDATA_SECT_OPEN: {
        enum XML_Error result;
        if (startCdataSectionHandler) startCdataSectionHandler(handlerArg);
#if 0
...
static enum XML_Error storeAtts(XML_Parser parser, const ENCODING *enc,
  ...
  if (n + nDefaultAtts > attsSize) {
    ...
    if (temp == NULL) return XML_ERROR_NO_MEMORY;
    atts = temp;
    if (n > oldAttsSize) XmlGetAttributes(enc, attStr, n, atts);
  }
  ...
  if (nPrefixes) {
    ...
    if ((nPrefixes << 1) >> nsAttsPower) { /* true for nsAttsPower = 0 */
      ...
      while (nPrefixes >> nsAttsPower++)
        ;
      if (nsAttsPower < 3) nsAttsPower = 3;
      nsAttsSize = (int)1 << nsAttsPower;
      temp = (NS_ATT *)REALLOC(nsAtts, nsAttsSize * sizeof(NS_ATT));
      if (!temp) return XML_ERROR_NO_MEMORY;
      nsAtts = temp;
    ...
    if (!version) { /* initialize version flags when version wraps around */
      ...
      for (j = nsAttsSize; j != 0;) nsAtts[--j].version = version;
    }
    ...
    for (; i < attIndex; i += 2) {
      ...
      if (s[-1] == 2) { /* prefixed */
        ...
        for (j = 0; j < b->uriLen; j++) {
          ...
          if (!poolAppendChar(&tempPool, c)) return XML_ERROR_NO_MEMORY;
        ...
        do { /* copies null terminator */
          const XML_Char c = *s;
          if (!poolAppendChar(&tempPool, *s)) return XML_ERROR_NO_MEMORY;
          uriHash = CHAR_HASH(uriHash, c);
        } while (*s++);
        ...
        { /* Check hash table for duplicate of expanded name (uriName).
          ...
          */
          ...
          while (nsAtts[j].version == version) {
            ...
            if (!step) step = PROBE_STEP(uriHash, mask, nsAttsPower);
...
static enum XML_Error addBinding(XML_Parser parser, PREFIX *prefix,
  ...
  if (namespaceSeparator) b->uri[len - 1] = namespaceSeparator;
  b->prefix = prefix;
  b->attId = attId;
    ...
    *eventEndPP = next;
    switch (tok) {
      case XML_TOK_CDATA_SECT_CLOSE:
        if (endCdataSectionHandler) endCdataSectionHandler(handlerArg);
#if 0
...
static enum XML_Error processXmlDecl(XML_Parser parser, int isGeneralTextEntity,
  ...
  if (storedEncName || storedversion) poolClear(&temp2Pool);
...
static enum XML_Error doProlog(XML_Parser parser, const ENCODING *enc,
  ...
  for (;;) {
    ...
    *eventEndPP = next;
    ...
    switch (role) {
      ...
      case XML_ROLE_DOCTYPE_NAME:
        if (startDoctypeDeclHandler) {
          ...
          if (!doctypeName) return XML_ERROR_NO_MEMORY;
          poolFinish(&tempPool);
          doctypePubid = NULL;
          handleDefault = XML_FALSE;
        }
      ...
      case XML_ROLE_DOCTYPE_PUBLIC_ID:
        ...
        declEntity = (ENTITY *)lookup(&dtd->paramEntities, externalSubsetName,
                                      sizeof(ENTITY));
        if (!declEntity) return XML_ERROR_NO_MEMORY;
#endif /* XML_DTD */
        ...
        if (startDoctypeDeclHandler) {
          ...
          doctypePubid =
              poolStoreString(&tempPool, enc, s + enc->minBytesPerChar,
                              next - enc->minBytesPerChar);
          if (!doctypePubid) return XML_ERROR_NO_MEMORY;
      ...
      alreadyChecked:
        if (dtd->keepProcessing && declEntity) {
          ...
          poolFinish(&dtd->pool);
          if (entityDeclHandler) handleDefault = XML_FALSE;
        }
        break;
        ...
        if (doctypeSysid || useForeignDTD) {
          ...
          if (paramEntityParsing && externalEntityRefHandler) {
            ENTITY *entity = (ENTITY *)lookup(
                &dtd->paramEntities, externalSubsetName, sizeof(ENTITY));
            if (!entity) return XML_ERROR_NO_MEMORY;
            if (useForeignDTD) entity->base = curBase;
      ...
      case XML_ROLE_ATTLIST_ELEMENT_NAME:
        ...
        if (!declElementType) return XML_ERROR_NO_MEMORY;
        goto checkAttListDeclHandler;
      case XML_ROLE_ATTRIBUTE_NAME:
        declAttributeId = getAttributeId(parser, enc, s, next);
        if (!declAttributeId) return XML_ERROR_NO_MEMORY;
        declAttributeIsCdata = XML_FALSE;
        declAttributeType = NULL;
        declAttributeIsId = XML_FALSE;
        goto checkAttListDeclHandler;
      ...
      case XML_ROLE_ATTRIBUTE_NOTATION_VALUE:
        if (dtd->keepProcessing && attlistDeclHandler) {
          ...
          if (!poolAppendString(&tempPool, prefix)) return XML_ERROR_NO_MEMORY;
          if (!poolAppend(&tempPool, enc, s, next)) return XML_ERROR_NO_MEMORY;
      ...
      case XML_ROLE_DOCTYPE_SYSTEM_ID:
        ...
        if (startDoctypeDeclHandler) {
          doctypeSysid =
              poolStoreString(&tempPool, enc, s + enc->minBytesPerChar,
                              next - enc->minBytesPerChar);
          if (doctypeSysid == NULL) return XML_ERROR_NO_MEMORY;
          poolFinish(&tempPool);
          handleDefault = XML_FALSE;
        }
#ifdef XML_DTD
      ...
      case XML_ROLE_ENTITY_SYSTEM_ID:
        if (dtd->keepProcessing && declEntity) {
          declEntity->systemId =
              poolStoreString(&dtd->pool, enc, s + enc->minBytesPerChar,
                              next - enc->minBytesPerChar);
          if (!declEntity->systemId) return XML_ERROR_NO_MEMORY;
      ...
      case XML_ROLE_ENTITY_COMPLETE:
        if (dtd->keepProcessing && declEntity && entityDeclHandler) {
          *eventEndPP = s;
          entityDeclHandler(handlerArg, declEntity->name, declEntity->is_param,
                            0, 0, declEntity->base, declEntity->systemId,
      ...
      case XML_ROLE_ENTITY_NOTATION_NAME:
        if (dtd->keepProcessing && declEntity) {
          ...
          if (!declEntity->notation) return XML_ERROR_NO_MEMORY;
          poolFinish(&dtd->pool);
      ...
      case XML_ROLE_NOTATION_NAME:
        ...
        if (notationDeclHandler) {
          ...
          if (!declNotationName) return XML_ERROR_NO_MEMORY;
          poolFinish(&tempPool);
          handleDefault = XML_FALSE;
        }
        break;
      ...
      case XML_ROLE_GROUP_OPEN:
        if (prologState.level >= groupSize) {
          if (groupSize) {
            ...
            if (!groupConnector) return XML_ERROR_NO_MEMORY;
          }
        }
      ...
      case XML_ROLE_GROUP_SEQUENCE:
        ...
        if (dtd->in_eldecl && elementDeclHandler) handleDefault = XML_FALSE;
        break;
      ...
      case XML_ROLE_DOCTYPE_NONE:
        if (startDoctypeDeclHandler) handleDefault = XML_FALSE;
        break;
      case XML_ROLE_ENTITY_NONE:
        if (dtd->keepProcessing && entityDeclHandler) handleDefault = XML_FALSE;
        break;
      case XML_ROLE_NOTATION_NONE:
        if (notationDeclHandler) handleDefault = XML_FALSE;
        break;
    ...
    if (handleDefault && defaultHandler) reportDefault(parser, enc, s, next);
  ...
  for (;;) {
    ...
    switch (tok) {
      ...
      case -XML_TOK_PROLOG_S:
        if (defaultHandler) {
          reportDefault(parser, encoding, s, next);
          if (ps_parsing == XML_FINISHED) return XML_ERROR_ABORTED;
        }
        *nextPtr = next;
        return XML_ERROR_NONE;
      case XML_TOK_NONE:
        *nextPtr = s;
        return XML_ERROR_NONE;
      case XML_TOK_PROLOG_S:
        if (defaultHandler) reportDefault(parser, encoding, s, next);
        break;
      case XML_TOK_PI:
    ...
    switch (tok) {
      case XML_TOK_PARAM_ENTITY_REF:
        ...
        if (isParamEntity || enc != encoding) {
          ...
          if (entity->open) {
            if (enc == encoding) eventPtr = entityTextPtr;
...
static int reportProcessingInstruction(XML_Parser parser, const ENCODING *enc,
  ...
  if (!processingInstructionHandler) {
    if (defaultHandler) reportDefault(parser, enc, start, end);
    return 1;
  }
...
static const XML_Char *getContext(XML_Parser parser) {
  ...
  if (dtd->defaultPrefix.binding) {
    int i;
    int len;
    if (!poolAppendChar(&tempPool, XML_T(ASCII_EQUALS))) return NULL;
  ...
  for (;;) {
    ...
    if (needSep && !poolAppendChar(&tempPool, CONTEXT_SEP)) return NULL;
    for (s = prefix->name; *s; s++)
      if (!poolAppendChar(&tempPool, *s)) return NULL;
    if (!poolAppendChar(&tempPool, XML_T(ASCII_EQUALS))) return NULL;
    len = prefix->binding->uriLen;
    if (namespaceSeparator) len--;
    for (i = 0; i < len; i++)
      if (!poolAppendChar(&tempPool, prefix->binding->uri[i])) return NULL;
    needSep = XML_TRUE;
  }
  ...
  if (!poolAppendChar(&tempPool, XML_T('\0'))) return NULL;
...
static XML_Bool setContext(XML_Parser parser, const XML_Char *context) {
  ...
  while (*context != XML_T('\0')) {
    if (*s == CONTEXT_SEP || *s == XML_T('\0')) {
      ENTITY *e;
      if (!poolAppendChar(&tempPool, XML_T('\0'))) return XML_FALSE;
      ...
      for (context = s + 1; *context != CONTEXT_SEP && *context != XML_T('\0');
           context++)
        if (!poolAppendChar(&tempPool, *context)) return XML_FALSE;
      if (!poolAppendChar(&tempPool, XML_T('\0'))) return XML_FALSE;
      ...
      if (!poolAppendChar(&tempPool, *s)) return XML_FALSE;
      s++;
    }
  }
  return XML_TRUE;
...
static XML_Bool FASTCALL poolGrow(STRING_POOL *pool) {
  ...
  if (pool->blocks && pool->start == pool->blocks->s) {
    ...
    if (pool->blocks == NULL) return XML_FALSE;
