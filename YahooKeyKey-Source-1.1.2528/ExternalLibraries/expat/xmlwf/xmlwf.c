...
static void XMLCALL defaultCharacterData(void *userData, const XML_Char *s,
                                         int len) {
  XML_DefaultCurrent((XML_Parser)userData);
}

static void XMLCALL defaultStartElement(void *userData, const XML_Char *name,
                                        const XML_Char **atts) {
  XML_DefaultCurrent((XML_Parser)userData);
}
...
static void XMLCALL nopCharacterData(void *userData, const XML_Char *s,
                                     int len) {}

static void XMLCALL nopStartElement(void *userData, const XML_Char *name,
                                    const XML_Char **atts) {}
...
static void XMLCALL markup(void *userData, const XML_Char *s, int len) {
  ...
  for (; len > 0; --len, ++s) puttc(*s, fp);
}

static void metaLocation(XML_Parser parser) {
  const XML_Char *uri = XML_GetBase(parser);
  if (uri) ftprintf((FILE *)XML_GetUserData(parser), T(" uri=\"%s\""), uri);
...
static void XMLCALL metaEntityDecl(void *userData, const XML_Char *entityName,
                                   int is_param, const XML_Char *value,
...
static int XMLCALL unknownEncoding(void *userData, const XML_Char *name,
                                   XML_Encoding *info) {
  int cp;
  ...
  for (i = 0; prefixU[i]; i++)
    if (name[i] != prefixU[i] && name[i] != prefixL[i]) return 0;
...
static int XMLCALL notStandalone(void *userData) { return 0; }
