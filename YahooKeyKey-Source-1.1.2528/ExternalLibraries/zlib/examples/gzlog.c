...
/* release lock */
local void unlock(int fd) { (void)flock(fd, LOCK_UN); }
...
local unsigned long make_ulg(unsigned char *buf) {
  ...
  for (n = 8; n < 32; n += 8) val += (unsigned long)(*buf++) << n;
  return val;
}
...
local off_t make_off(unsigned char *buf) {
  ...
  val = (off_t)(*buf++);
  for (n = 8; n < 64; n += 8) val += (off_t)(*buf++) << n;
  return val;
}
...
void *gzlog_open(char *path) {
  ...
  if (read(log->fd, temp, 12) != 12 || temp[0] != 0x1f || temp[1] != 0x8b ||
      temp[2] != 8 || (temp[3] & 4) == 0) {
    log_clean(log);
    return NULL;
  }
  ...
  good = 0;
  while (xlen) {
    if (xlen < 4 || read(log->fd, temp, 4) != 4) break;
    sub_len = temp[2];
    sub_len += temp[3] << 8;
    xlen -= 4;
...
int gzlog_write(void *obj, char *data, size_t len) {
  ...
  /* check object */
  log = (gz_log *)obj;
  if (log == NULL || log->id != GZLOGID) return 1;

  /* write stored blocks until all of the input is written */
  do {
    some = MAX_BLOCK - log->stored;
    if (some > len) some = len;
    if (write(log->fd, data, some) != some) return 1;
    ...
    if (log->stored == MAX_BLOCK) {
      (void)lseek(log->fd, log->last_off, SEEK_SET);
      temp[0] = 0;
      dice_ulg(log->stored + ((unsigned long)(~log->stored) << 16), temp + 1);
      if (write(log->fd, temp, 5) != 5) return 1;
...
local int recomp(gz_log *log) {
  ...
  strm.next_out = out;
  while (len >= 5) {
    if (strm.next_in[0] != 0) break;
    strm.avail_in = strm.next_in[1] + (strm.next_in[2] << 8);
    strm.next_in += 5;
    len -= 5;
    if (strm.avail_in != 0) {
      if (len < strm.avail_in) break;
      len -= strm.avail_in;
  ...
  if (write(log->fd, temp, 13) != 13) return 1;
  ...
  if (write(log->fd, temp, 16) != 16) return 1;
  return 0;
}
