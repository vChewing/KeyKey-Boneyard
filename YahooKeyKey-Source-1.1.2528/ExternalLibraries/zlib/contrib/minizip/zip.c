...
zlib_filefunc_def* pzlib_filefunc_def;
{
  ...
  if (append == APPEND_STATUS_ADDINZIP) {
    ...
    if (central_pos == 0) err = ZIP_ERRNO;
...
uLong crcForCrypting;
{
  ...
  ziplocal_putValue_inmemory(zi->ci.central_header + 10, (uLong)zi->ci.method,
                             2);
  ziplocal_putValue_inmemory(zi->ci.central_header + 12, (uLong)zi->ci.dosDate,
                             4);
  ...
  if (err == ZIP_OK)
    err = ziplocal_putValue(&zi->z_filefunc, zi->filestream, (uLong)zi->ci.flag,
                            2);
  ...
  if ((err == ZIP_OK) && (zi->ci.method == Z_DEFLATED) && (!zi->ci.raw)) {
    ...
    err = deflateInit2(&zi->ci.stream, level, Z_DEFLATED, windowBits, memLevel,
                       strategy);
...
extern int ZEXPORT zipOpenNewFileInZip2(file, filename, zipfi, extrafield_local,
                                        size_extrafield_local,
                                        extrafield_global,
                                        size_extrafield_global, comment, method,
                                        level, raw)
...
extern int ZEXPORT zipOpenNewFileInZip(file, filename, zipfi, extrafield_local,
...
uLong crc32;
{
  ...
  if (err == ZIP_OK) {
    ...
    if (err == ZIP_OK) /* compressed size, unknown */
      err = ziplocal_putValue(&zi->z_filefunc, zi->filestream, compressed_size,
                              4);
