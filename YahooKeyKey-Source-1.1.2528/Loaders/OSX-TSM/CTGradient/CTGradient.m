...
+ (id)gradientWithBeginningColor:(NSColor *)begin endingColor:(NSColor *)end {
  ...
  [[begin colorUsingColorSpaceName:NSCalibratedRGBColorSpace]
      getRed:&color1.red
       green:&color1.green
        blue:&color1.blue
       alpha:&color1.alpha];

  [[end colorUsingColorSpaceName:NSCalibratedRGBColorSpace]
      getRed:&color2.red
       green:&color2.green
        blue:&color2.blue
       alpha:&color2.alpha];
...
- (CTGradient *)addColorStop:(NSColor *)color atPosition:(float)position {
  ...
  // put the components of color into the newGradientElement - must make sure it
  // is a RGB color (not Gray or CMYK)
  [[color colorUsingColorSpaceName:NSCalibratedRGBColorSpace]
      getRed:&newGradientElement.red
...
- (CTGradient *)removeColorStopAtPosition:(float)position {
  ...
  CTGradientElement removedElement =
      [newGradient removeElementAtPosition:position];

  if (isnan(removedElement.position))
    [NSException raise:NSRangeException
                format:
                    @"-[%@ removeColorStopAtPosition:]: no such colorStop at "
                    @"position (%f)",
                    [self class], position];
...
- (CTGradient *)removeColorStopAtIndex:(unsigned)index {
  ...
  if (isnan(removedElement.position))
    [NSException
        ...
        format:@"-[%@ removeColorStopAtIndex:]: index (%i) beyond bounds",
               [self class], index];
...
- (void)fillRect:(NSRect)rect angle:(float)angle radius:(float)radius {
...
#endif
  CGShadingRef myCGShading = CGShadingCreateAxial(
      colorspace, startPoint, endPoint, gradientFunction, false, false);
  ...
  // CGContextClipToRect (currentContext, *(CGRect *)&rect);	//This is where
  // the action happens
...
- (void)radialFillRect:(NSRect)rect {
  ...
  CGShadingRef myCGShading =
      CGShadingCreateRadial(colorspace, startPoint, startRadius, endPoint,
                            endRadius, gradientFunction, true, true);
  ...
  CGContextDrawShading(currentContext,
                       myCGShading);  // This is where the action happens
...
- (void)setBlendingMode:(CTGradientBlendingMode)mode;
{
  ...
  CGFunctionCallbacks evaluationCallbackInfo = {
      0, evaluationFunction,
      NULL};  // Version, evaluator function, cleanup function

  static const float input_value_range[2] = {
      0, 1};  // range  for the evaluator input
  static const float output_value_ranges[8] = {
      0, 1, 0, 1,
      0, 1, 0, 1};  // ranges for the evaluator output (4 returned values)

  gradientFunction = CGFunctionCreate(
      &elementList,          // the two transition colors
...
- (void)addElement:(CTGradientElement *)newElement {
  if (elementList == nil ||
      newElement->position <
          elementList->position)  // inserting at beginning of list
  {
...
void linearEvaluation(void *info, const float *in, float *out) {
  ...
  if (*(CTGradientElement **)info ==
      nil)  // if elementList is empty return clear color
  {
  ...
  if (position <=
      color1
          ->position)  // Make all below color color1's position equal to color1
  {
    out[0] = color1->red;
    out[1] = color1->green;
    out[2] = color1->blue;
    out[3] = color1->alpha;
  } else if (position >= color2->position)  // Make all above color color2's
                                            // position equal to color2
  {
    out[0] = color2->red;
    out[1] = color2->green;
    out[2] = color2->blue;
  ...
  {
    // adjust position so that it goes from 0 to 1 in the range from color 1 &
    // 2's position
    position =
        (position - color1->position) / (color2->position - color1->position);
...
void transformHSV_RGB(float *components)  // H,S,B -> R,G,B
{
  ...
  float f = H / 60 - Hi, p = V * (1 - S), q = V * (1 - f * S),
        t = V * (1 - (1 - f) * S);
...
void resolveHSV(
    float *color1,
    float *color2)  // H value may be undefined (i.e. graycale color)
